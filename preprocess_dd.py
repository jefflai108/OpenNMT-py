...
def check_existing_pt_files(opt):
    ...
    # or mixing them up.
    for t in ['train0', 'train1', 'valid0', 'valid1', 'vocab']:
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    assert corpus_type in ['train0', 'train1', 'valid0', 'valid1']

    if corpus_type == 'train0':
        src_corpus = opt.train_src
        tgt_corpus = opt.train_tgt
    elif corpus_type == 'train1':
        src_corpus = opt.train_src1
        tgt_corpus = opt.train_tgt1
    elif corpus_type == 'valid0':
...
def _get_fields(data_type, train_src, train_tgt):
    ...
    src_nfeats = inputters.get_num_features(
        data_type, train_src, 'src')
    tgt_nfeats = inputters.get_num_features(
        data_type, train_tgt, 'tgt')
